...
```swift
dependencies: [
    .package(url: "https://github.com/flagkit/flagkit-swift.git", from: "1.0.0")
]
```
...
let options = FlagKitOptions.Builder(apiKey: "sdk_your_api_key")
    ...
    .isLocal(false)                                 // Use local dev server (localhost:8200)
    .build()
...
For local development, enable `isLocal` to connect to a local FlagKit server running on `http://localhost:8200`:
...
All FlagKit types are designed for concurrent access using Swift's actor isolation. The `FlagKitClient` is an actor, ensuring thread-safe operations.
